// [C4-STEP-6+7] Game: Step 6 (SFX/Haptik-Events via main.js, Undo/Reset) + Snapshot Export/Import
...
export function undo(count = 1) {
  ...
  while (count-- > 0 && history.length > 0) {
    ...
    try {
      ...
      mv.mesh.geometry?.dispose?.(); mv.mesh.material?.dispose?.();
    } catch {}
    if (boardState[mv.row][mv.col] !== 0) {
      boardState[mv.row][mv.col] = 0;
      movesCount = Math.max(0, movesCount - 1);
      currentPlayer = mv.player; // der Spieler ist wieder dran, der den Zug gemacht hatte
      undone++;
    }
  }
  if (undone > 0) {
...
export function importSnapshot(snap) {
  ...
  const gridMeshes = Array.from({ length: rows }, () => Array(cols).fill(null));
  ...
  // Historie wiederherstellen (Mesh-Verknüpfung)
  if (Array.isArray(snap.history)) {
    for (const mv of snap.history) {
      const m = gridMeshes[mv.row]?.[mv.col] || null;
      if (m) history.push({ row: mv.row, col: mv.col, player: mv.player, mesh: m });
    }
  }
